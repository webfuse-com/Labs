    ...
    return new Promise(resolve => {
        createServer(async (req, res) => {
            ...
            try {
                ...
                const assetFilePath = await readDoc(req.url);
                if(assetFilePath)
                    return end(assetFilePath, MIME[extname(req.url)]);

                const distFilePath = await readDoc(req.url, DIST_PATH);
                if(distFilePath)
                    return end(distFilePath, MIME[extname(req.url)]);

                end(await readDoc("/assets/404.html"), null, 404)
            } catch(err) {
                end(null, null, 500);
