    ...
    return new Promise(resolve => {
        createServer(async (req, res) => {
            ...
            try {
                ...
                const isContentFile = req.url === "/_assets/content.html";
                ...
                if (["/newtab.html", "/popup.html"].includes(req.url) || isContentFile) {
                    ...
                    distFile = injectPriorityScriptRaw(distFile, `
						browser.webfuseSession.env = ${JSON.stringify(Object.fromEntries((JSON.parse(((await readDoc("manifest.json", DIST_PATH)) ?? "{}").toString()).env ?? []).map(item => [item.key, item.value])))};
					`);
